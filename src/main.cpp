...
static const twai_general_config_t g_config = { .mode = TWAI_MODE_NORMAL, .tx_io = TX_GPIO_NUM, .rx_io = RX_GPIO_NUM,        \
 ...
 static  twai_message_t tx_msg = { .flags = TWAI_MSG_FLAG_EXTD, .identifier = 0xAABB, .data_length_code = 8,
...
SemaphoreHandle_t CAN_ctrl_task_sem;
...
const char* SDO_ret_to_string(CO_SDO_return_t ret) {
	switch(ret) {
		case CO_SDO_RT_waitingLocalTransfer:      return "Waiting local transfer";
		case CO_SDO_RT_uploadDataBufferFull:      return "Upload data buffer full";
		case CO_SDO_RT_transmittBufferFull:       return "Transmit buffer full";
		case CO_SDO_RT_blockDownldInProgress:     return "Block download in progress";
		case CO_SDO_RT_blockUploadInProgress:     return "Block upload in progress";
		case CO_SDO_RT_waitingResponse:           return "Waiting response";
		case CO_SDO_RT_ok_communicationEnd:       return "OK - Communication end";
		case CO_SDO_RT_wrongArguments:            return "Wrong arguments";
		case CO_SDO_RT_endedWithClientAbort:      return "Ended with client abort";
		case CO_SDO_RT_endedWithServerAbort:      return "Ended with server abort";
...
std::vector<uint8_t> discoverNodesBySDO(uint8_t firstId, uint8_t lastId, uint16_t index, uint8_t subIndex,
                                        std::vector<uint32_t>* outValues) {
    std::vector<uint8_t> found;
    if (firstId < 1) firstId = 1;
    if (lastId > 127) lastId = 127;
    if (lastId < firstId) return found;

    // Sanidade mínima do CO
    if (CO == nullptr || CO->CANmodule == nullptr) {
        return found;
    }

    for (uint8_t id = firstId; id <= lastId; ++id) {
        uint32_t val = 0;
        CO_SDO_abortCode_t ac = readSDOUint32(CO->SDOclient, firstId, index, subIndex, &val);

        if (ac == CO_SDO_AB_NONE) {
            // Sucesso: nó existe e respondeu ao SDO
            found.push_back(id);
            if (outValues) outValues->push_back(val);
            ESP_LOGI("TESTE", "Node %u: OK - 0x%04X:%02X = 0x%08X",
                     (unsigned)id, (unsigned)index, (unsigned)subIndex, (unsigned)val);
        } else {
            // Falha/timeout/abort – assumimos que não há dispositivo nesse ID
            ESP_LOGI(" TESTE ", "Node %u: SDO abort=0x%08X em 0x%04X:%02X",
                     (unsigned)id, (unsigned)ac, (unsigned)index, (unsigned)subIndex);
        }
...
std::vector<uint8_t> discoverNodesByDeviceType(uint8_t firstId, uint8_t lastId, std::vector<uint32_t>* deviceTypes) {
    return discoverNodesBySDO(firstId, lastId, 0x1000, 0x00, deviceTypes);
}

// static void teste(void* arg){
// 	Serial.println("Scanning SDO (0x1000:0)...");
// 	CO_LOCK_OD(CO->CANmodule);
// 	CO_SDO_abortCode_t ac = read_SDO(CO->SDOclient, 9, 0x1000, 0x00, NULL, 0, NULL);
// 	if (ac != CO_SDO_AB_NONE) {
// 		ESP_LOGI("TESTE", "SDO read failed: 0x%08X", (unsigned)ac);
// 	} else {
// 		ESP_LOGI("TESTE", "SDO read initiated");
// 	}
...
void setup()
{
  ...
  esp_log_level_set("*", ESP_LOG_DEBUG);
	...
	xTaskCreatePinnedToCore(CAN_ctrl_task, "TWAI_ctrl", 2 * 4096, NULL, CAN_CTRL_TASK_PRIO, NULL, tskNO_AFFINITY);
	vTaskDelay(pdMS_TO_TICKS(100));
	xTaskCreatePinnedToCore(CO_main, "CO_MAIN", 2 * 6000, NULL, CANOPEN_TASK_PRIO, NULL, tskNO_AFFINITY);
	vTaskDelay(pdMS_TO_TICKS(1000));
	xTaskCreatePinnedToCore(serialCommandTask, "SERIAL_CMD", 4096, NULL, 1, NULL, tskNO_AFFINITY);
	vTaskDelay(pdMS_TO_TICKS(1000));
	// xTaskCreatePinnedToCore(teste, "teste", 8192, NULL, 1, NULL, tskNO_AFFINITY);
	// vTaskDelay(pdMS_TO_TICKS(1000));
